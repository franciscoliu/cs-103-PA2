"""
Creates a Transaction object which connects with the sqlite database
to save information such as transactions and categories
"""
...
def to_cat_dict(cat_tuple):
    ...
    cat = {'rowid': cat_tuple[0], 'item_number': cat_tuple[1], 'amount': cat_tuple[2],
...
def summarize_by_date_helper(cat_tuple):
    ''' transform list of dates to list of dictionaries '''
    cat = {'date': cat_tuple[0], 'total_amount_transaction': cat_tuple[1]}
    return cat


def summarize_by_category_helper(cat_tuple):
    ''' transform list of categories to list of dictionaries '''
...
class Transaction:
    ...
    def add(self, item):
        ...
        cur.execute("INSERT INTO transactions VALUES(?,?,?,?,?)",
                    (item['item_number'], item['amount'], item['category'],
    ...
    def summarize_by_date(self):
        ''' summarizes transactions by date and returns amount sum on each date '''
        con = sqlite3.connect(self.dbfile)
        cur = con.cursor()
    ...
    def summarize_by_month(self):
        ''' summarizes transactions by month and returns amount sum on each date '''
        con = sqlite3.connect(self.dbfile)
        cur = con.cursor()
    ...
    def summarize_by_year(self):
        ''' summarizes transactions by year and returns amount sum on each date '''
        con = sqlite3.connect(self.dbfile)
        cur = con.cursor()
    ...
    def summarize_by_category(self):
        ''' summarizes transactions by category and returns amount sum on each date '''
        con = sqlite3.connect(self.dbfile)
        cur = con.cursor()
